    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Padding(
          ...
          child: Column(
            children: [
              isMobile ? _buildVerticalLayout() : _buildResponsiveTable(),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              SizedBox(
                width: MediaQuery.of(context).size.width * 0.03,  // Ekran genişliğinin %8'i kadar genişlik
                height: MediaQuery.of(context).size.height * 0.05, // Ekran yüksekliğinin %5'i kadar yükseklik
  ...
  void _handleSubmit() {
    ...
    for (int i = 0; i < selectedValues.length; i++) {
      if (selectedValues[i] == null) {
        ...
        // Başlık ve basılan butonun değerini buraya ekliyoruz
        String title = 'Başlık $i'; // Başlık metni
        String response = 'Değer ${selectedValues[i]}'; // Buton değeri
        responses.add('$title: $response');
      }
    }

    if (allFilled) {
      // Tüm veriler doldurulmuşsa, terminale başlık ve değerleri yazdır
      print("Tüm veriler doldurulmuş:");
      for (String response in responses) {
        print(response);
      }
      Navigator.pushReplacementNamed(context, '/M25');
    } else {
      // Eksik veri varsa, uyarı mesajı göster
      showDialog(
        context: context,
        builder: (context) {
          return AlertDialog(
